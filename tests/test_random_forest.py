...
def test_dominant_feature():
    ...
    forest = RandomForest("Classification", n_estimators=100, criterion=Gini_index, bootstrap=True)
    forest.fit(X, Y)

    # Create data for predict
    prediction_data = np.random.randint(0, 100, (n, m + 1))
    ...
    for i, el in enumerate(pred):
        assert(el, prediction_data[i, -1], f"The data for prediction should be equal to the data in the last row of prediction_data, as it is a dominant feature")
